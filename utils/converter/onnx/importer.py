...
class OnnxImporter:
    ...
    def ScatterElements_13(self, func_list, n):
        ...
        wout = fork_name(n.input[1])+"_where"
        del where_func.output[:]
        where_func.output.extend([wout])
        self._shape_output[wout] = indices_shape
        func_list.append(where_func)

        # handle scatterElements
        sa_func = self.generate_default_function("ScatterAdd", n)
        sap = sa_func.scatter_add_param
        sap.axis = axis
        sa_func.input[1] = wout
    ...
    def Shape(self, func_list, n):
        ...
        output_shape = input_shape[start:end]
